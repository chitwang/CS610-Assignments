...
int main() {
  uint64_t SIZE = N * N * N;
    std::cout << "The program will run CPU version and a UVM + shared memory CUDA Kernel for stencil\n#############Results##############\n";
  ...
  double *managed_in;
  double *managed_out;
  cudaCheckError(cudaMallocManaged(&managed_in, SIZE * sizeof(double)));
  cudaCheckError(cudaMallocManaged(&managed_out, SIZE * sizeof(double)));

  for(uint64_t i=0;i<SIZE;i++){
    managed_in[i] = static_cast<double>(rand())/RAND_MAX;
  }

  double clkbegin = rtclock();
  stencil(managed_in, h_out);
  ...
  kernel2<<<grid2, block2>>> (managed_in, managed_out);
  ...
  check_result(managed_out, h_out, N);
  
  // Free memory
  cudaCheckError(cudaFree(managed_in));
  cudaCheckError(cudaFree(managed_out));
